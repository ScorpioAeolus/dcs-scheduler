...
abstract class AbstractLockConfiguration implements ImportAware {
    ...
    public void setImportMetadata(AnnotationMetadata importMetadata) {
        this.annotationAttributes = AnnotationAttributes.fromMap(
                importMetadata.getAnnotationAttributes(EnableDcsScheduling.class.getName(), false));
        if (this.annotationAttributes == null) {
            throw new IllegalArgumentException(
                    "@EnableDcsScheduling is not present on importing class " + importMetadata.getClassName());
        }
    }
